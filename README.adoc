...
Generate gpg key with command `gpg --export-secret-keys --armor F207B5A1 > my.gpg`. use `cat my.gpg` to output secret key in the console and copy the output as the vale of `SIGNING_KEY`.
`F207B5A1` is last 8 characters in the public key, you can get public key by `gpg --list-keys`
